...
- **Visible and cloaked windows** — including windows that set themselves "excluded from screen capture"
- **Virtual audio and video devices** — virtual cameras (deepfake vectors) and virtual audio cables (voice-routing vectors)
- **Remote-access and tunneling tools** — TeamViewer, AnyDesk, Parsec, RustDesk, ngrok, cloudflared, tailscale, etc.
...
- **No termination of processes.** Dayside is read-only on your machine — it shows you what's running, but you close things yourself through your OS's own task manager.
...
- **No hiding.** Dayside itself is always visible to the interviewer during the screen-share. It does not exclude itself from screen capture. It does not run in the system tray. The whole premise of the tool depends on it being visible to both parties.
...
Pre-built binaries for Windows and macOS are published on the [Releases page](../../releases). Download, extract, run.
...
See [FOR_EMPLOYERS.md](FOR_EMPLOYERS.md) for a short guide on responsible deployment. Short version: give candidates advance notice, offer an alternative for those who can't or won't run it, train your interviewers to treat flags as conversation starters rather than verdicts, and accommodate candidates who rely on assistive software. In some jurisdictions — notably the EU, UK, and several US states — additional compliance steps apply. **You are responsible for your own legal compliance. This project is a tool, not legal advice.**
...
**DLL-loading hardening (Windows).** Early versions of Wails v2 were vulnerable to a DLL-hijacking pattern where `uxtheme.dll` could be loaded from the application directory before the process could restrict its DLL search path. This was [reported and fixed by the Wails team](https://github.com/wailsapp/wails/pull/4207) and Dayside builds against the patched Wails version (v2.10.0 or later). On top of that, `main.go` explicitly calls `windows.SetDefaultDllDirectories(LOAD_LIBRARY_SEARCH_SYSTEM32)` at startup as defense in depth. Thanks to the Wails maintainers for the upstream fix.
...
**Reporting issues.** If you find a security issue — particularly anything that could cause Dayside to mis-identify a legitimate process as malicious, or anything that could be exploited to access data beyond what's documented — please report it privately via GitHub's security advisory feature rather than a public issue.

## License

Apache License 2.0. See [LICENSE](LICENSE). Dayside is provided as-is with no warranty. You use it at your own risk.

## Author

Published under a pseudonym. Contact via GitHub issues or the security advisory channel. For legal matters, please go through GitHub's standard abuse/copyright process.
